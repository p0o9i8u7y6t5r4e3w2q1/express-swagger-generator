      ...
      for (let j in comments[i]) {
        ...
        if (title == "param") {
          ...
          if (!schema) {
            ...
            // *** add my condition here
          } else if (schema.type == "array") {
            properties.collectionFormat = "multi";
            if (schema.items.$ref == "#/definitions/enum") {
              let parsedEnum = parseEnums(comments[i][j]["description"]);
              schema.items = { type: parsedEnum.type, enum: parsedEnum.enums };
            }
            properties = Object.assign(properties, schema);
